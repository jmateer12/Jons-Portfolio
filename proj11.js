function printcatfact(){
    ...
    fetch("https://catfact.ninja/fact")
        //when we get the response interpret it as JSON
        .then(response => response.json())
        //Take the data from this fetch result and display it to the page using fetchtext
        .then(data => {
            document.getElementById("fetchtext").innerText = data.fact;
        })
        //If an error occurs print the error to the screen using fetchtext
        .catch(error => {
            document.getElementById("fetchtext").innerText = "Error fetching cat fact";
        });
}

function printfakename(){
    //Something I noticed was that xhr is a lot more difficult to setup and harder to understand
    //Setup xhr request with fake name generator API
    let xhr = new XMLHttpRequest();
    xhr.open("GET", "https://randomuser.me/api/", true);
    
    //When response has loaded we check for a successful status then parse text to get the first object being the name
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            document.getElementById("xmltext").innerText = fullName;
        } else {
            document.getElementById("xmltext").innerText = "Error fetching name";
        }
    };

    //Displays an error message to the xmltext if there is a network problem
    xhr.onerror = function () {
        document.getElementById("xmltext").innerText = "Network error";
    };
